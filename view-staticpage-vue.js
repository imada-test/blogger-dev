(function () {

  blog_id="8624434127519986313";

  /* ---------------------------------------------------------
   * ① Bootstrap Icons および スタイル（CSS）の動的注入
   * --------------------------------------------------------- */
  if (!document.getElementById("bs-icons-css")) {
    const bsIconsLink = document.createElement("link");
    bsIconsLink.id = "bs-icons-css";
    bsIconsLink.rel = "stylesheet";
    bsIconsLink.href = "https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css";
    document.head.appendChild(bsIconsLink);
  }

  const customStyle = document.createElement("style");
  customStyle.textContent = `
    /* トースト通知 */
    .toast {
      position: fixed;
      right: 20px;
      bottom: 20px;
      background: #333;
      color: #fff;
      padding: 12px 18px;
      border-radius: 6px;
      font-size: 14px;
      opacity: 0;
      transition: opacity 0.4s ease;
      z-index: 10000;
    }
    .toast.show {
      opacity: 1;
    }

    /* ラベルボックス */
    .label-row {
      display: flex;
      align-items: center;
      gap: 0.8rem;
      margin-top: 12px;
    }
    .post-labels-title {
      font-weight: 600;
      font-size: 0.95rem;
      padding: 0.25rem 0.6rem;
      background-color: #eeeeee;
      border: 1px solid #bbbbbb;
      border-radius: 6px;
      white-space: nowrap;
    }
    .label-box {
      background-color: #fff9d6;
      border: 1px solid #f0e6b8;
      padding: 10px 12px;
      border-radius: 10px;
      display: inline-flex;
      flex-wrap: wrap;
      gap: 0.4rem;
      cursor: pointer;
    }
    .label-pill {
      display: inline-flex;
      align-items: center;
      padding: 0.15rem 0.7rem;
      border-radius: 999px;
      border: 1px solid #cc7a8a;
      background-color: #ffe5ec;
      color: #333;
      font-size: 0.85rem;
      font-weight: 600;
      white-space: nowrap;
    }
    .label-pill:hover {
      background-color: #ffd6e5;
      border-color: #d45c78;
    }

    /* 投稿タイトル用ピル型バッジ */
    .badge-pill-important {
      display: inline-flex;
      align-items: center;
      padding: 0.1rem 0.5rem;
      border-radius: 999px;
      background-color: #ff0000;
      color: #000000;
      border: 1px solid #000000;
      font-size: 0.75rem;
      font-weight: bold;
      margin-right: 0.4rem;
      vertical-align: middle;
      white-space: nowrap;
    }
    .badge-pill-basic {
      display: inline-flex;
      align-items: center;
      padding: 0.1rem 0.5rem;
      border-radius: 999px;
      background-color: #ffffe0;
      color: #000000;
      border: 1px solid #000000;
      font-size: 0.75rem;
      font-weight: bold;
      margin-right: 0.4rem;
      vertical-align: middle;
      white-space: nowrap;
    }

    /* モーダルダイアログ */
    .custom-modal-overlay {
      position: fixed;
      top: 0;
      left: 0;
      width: 100vw;
      height: 100vh;
      background-color: rgba(0, 0, 0, 0.5);
      display: flex;
      justify-content: center;
      align-items: center;
      z-index: 9999;
    }
    .custom-modal-content {
      background: #ffffff;
      padding: 24px;
      border-radius: 12px;
      max-width: 520px;
      width: 90%;
      box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15);
    }
    .custom-modal-content ol {
      padding-left: 1.2rem;
      margin-top: 1rem;
      margin-bottom: 1.5rem;
      line-height: 1.6;
    }
    .custom-modal-content li {
      margin-bottom: 0.5rem;
    }
  `;
  document.head.appendChild(customStyle);

  /* ---------------------------------------------------------
   * ② トースト表示用関数
   * --------------------------------------------------------- */
  function showToast(message) {
    const toast = document.createElement("div");
    toast.className = "toast";
    toast.textContent = message;
    document.body.appendChild(toast);

    setTimeout(() => toast.classList.add("show"), 50);
    setTimeout(() => {
      toast.classList.remove("show");
      setTimeout(() => toast.remove(), 400);
    }, 2000);
  }

  /* ---------------------------------------------------------
   * ③ Vue 読み込み
   * --------------------------------------------------------- */
  const vueScript = document.createElement("script");
  vueScript.src = "https://unpkg.com/vue@3/dist/vue.global.prod.js";
  document.head.appendChild(vueScript);

  /* ---------------------------------------------------------
   * ④ Vue と DOM の準備待ち
   * --------------------------------------------------------- */
  const waitReady = setInterval(() => {
    if (!window.Vue) return;

    const targets = document.querySelectorAll("feed-section");
    if (targets.length === 0) return;

    clearInterval(waitReady);

    const { createApp } = Vue;

    /* ---------------------------------------------------------
     * ⑤ LabelBox コンポーネント (Options API)
     * --------------------------------------------------------- */
    const LabelBox = {
      name: "LabelBox",
      props: {
        labels: { type: Array, required: true }
      },
      methods: {
        copyLabels() {
          const text = this.labels.join(",") + ",";
          navigator.clipboard.writeText(text).then(() => {
            if (location.protocol !== "file:") {
              showToast("コピーしました");
            } else {
              alert("コピーしました: " + text);
            }
          });
        }
      },
      template: `
        <div class="label-box" @click="copyLabels">
          <span v-for="(label, index) in labels"
                :key="index"
                class="label-pill">
            {{ label }}
          </span>
        </div>
      `
    };

    /* ---------------------------------------------------------
     * ⑥ FeedSection コンポーネント（Options API 版）
     * --------------------------------------------------------- */
    const FeedSection = {
      name: "FeedSection",
      components: { LabelBox },

      props: {
        label: { type: String, required: true },
        overviewSub: { type: String, default: "概要" },
        importantSub: { type: String, default: "重要" },
        basicSub: { type: String, default: "基本" },
        overviewLimit: { type: Number, default: 1 },
        latestLimit: { type: Number, default: 5 },
        importantLimit: { type: Number, default: 20 },
        basicLimit: { type: Number, default: 20 }
      },

      data() {
        return {
          overview: [],
          overviewTotal: 0,
          latest: [],
          latestTotal: 0,
          important: [],
          importantTotal: 0,
          basic: [],
          basicTotal: 0,
          isHelpModalOpen: false,
          helpModalText: ""
        };
      },

      methods: {
        formatDate(dateObj) {
          if (!dateObj || !(dateObj instanceof Date)) return "";
          const yyyy = dateObj.getFullYear();
          const mm = String(dateObj.getMonth() + 1).padStart(2, "0");
          const dd = String(dateObj.getDate()).padStart(2, "0");
          return `${yyyy}/${mm}/${dd}`;
        },

        makeSearchURL(labels) {
          return `/search/label/${labels.join("+")}`;
        },

        copyAndNavigate(labels) {
          const text = labels.join(",") + ",";
          const targetUrl = "https://www.blogger.com/blog/posts/" + blog_id;
          
          navigator.clipboard.writeText(text).then(() => {
            if (location.protocol !== "file:") {
              showToast("コピーしました");
            }
            window.location.href = targetUrl;
          }).catch(err => {
            console.error("クリップボードへのコピーに失敗しました", err);
            window.location.href = targetUrl;
          });
        },

        openHelpModal(text) {
          this.helpModalText = text;
          this.isHelpModalOpen = true;
        },

        closeHelpModal() {
          this.isHelpModalOpen = false;
        },

        async fetchFeed(labels) {
          const base = `/feeds/posts/summary`;
          const path = labels.length ? "/-/" + labels.join("/") : "";
          const url = `${base}${path}?alt=json`;

          try {
            const res = await fetch(url);
            const data = await res.json();
            const entries = (data.feed && data.feed.entry) ? data.feed.entry : [];
            const totalResults = data.feed && data.feed.openSearch$totalResults 
              ? parseInt(data.feed.openSearch$totalResults.$t, 10) 
              : entries.length;

            const items = entries.map(e => {
              const linkObj = e.link.find(l => l.rel === "alternate");
              const categories = e.category ? e.category.map(c => c.term) : [];
              return {
                id: e.id.$t,
                title: e.title.$t,
                link: linkObj ? linkObj.href : "#",
                published: new Date(e.published.$t),
                content: e.summary ? e.summary.$t : "",
                categories: categories
              };
            });

            return { items, totalResults };

          } catch (err) {
            console.error("Feed error:", err);
            return { items: [], totalResults: 0 };
          }
        },

        sortByDate(items) {
          return items.sort((a, b) => b.published - a.published);
        },

        async loadAllFeeds() {
          const overviewData = await this.fetchFeed([this.label, this.overviewSub]);
          this.overviewTotal = overviewData.totalResults;
          this.overview = this.sortByDate(overviewData.items).slice(0, this.overviewLimit);

          const latestData = await this.fetchFeed([this.label]);
          this.latestTotal = latestData.totalResults;
          this.latest = this.sortByDate(latestData.items).slice(0, this.latestLimit);

          const importantData = await this.fetchFeed([this.label, this.importantSub]);
          this.importantTotal = importantData.totalResults;
          this.important = this.sortByDate(importantData.items).slice(0, this.importantLimit);

          const basicData = await this.fetchFeed([this.label, this.basicSub]);
          this.basicTotal = basicData.totalResults;
          this.basic = this.sortByDate(basicData.items).slice(0, this.basicLimit);
        }
      },

      created() {
        this.loadAllFeeds();
      },

      /* ---------------------------------------------------------
       * Bootstrap 5 版テンプレート
       * --------------------------------------------------------- */
      template: `
        <div class="container my-4">

          <!-- 概要 -->
          <div class="mb-4">
            <div class="p-3 bg-light border-start border-primary rounded">
              <h2>{{ label }} についての概要</h2>
            </div>

            <div class="label-row">
              <span class="post-labels-title">投稿ラベル</span>
              <label-box :labels="[label, overviewSub]"></label-box>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3">
              <button type="button"
                      class="btn btn-outline-success btn-sm rounded-pill"
                      @click="copyAndNavigate([label, overviewSub])">
                新しい投稿記事を書いてここに表示する
              </button>
              <button type="button"
                      class="btn btn-outline-secondary btn-sm rounded-pill"
                      @click="openHelpModal(label + ' と ' + overviewSub)">
                ヘルプ
              </button>
            </div>

            <div v-if="overviewTotal > overviewLimit">
              <a :href="makeSearchURL([label, overviewSub])"
                 class="btn btn-outline-primary btn-sm rounded-pill mb-3">
                 一覧を見る
              </a>
            </div>

            <div class="row row-cols-1 row-cols-md-2 g-3">
              <div v-for="item in overview" :key="item.id" class="col">
                <div class="card h-100">
                  <div class="card-body">
                    <h5 class="card-title">
                      <a :href="item.link" class="fw-bold text-primary text-decoration-none">
                        <span v-if="item.categories.includes('重要')" class="badge-pill-important">重要</span>
                        <span v-if="item.categories.includes('基本')" class="badge-pill-basic">基本</span>
                        {{ item.title }}
                      </a>
                    </h5>
                    <div class="text-muted small mb-2">
                      <i class="bi bi-calendar3 me-1"></i>{{ formatDate(item.published) }}
                    </div>
                    <p class="card-text small text-secondary">{{ item.content.slice(0, 80) }}...</p>
                  </div>
                </div>
              </div>
            </div>

            <a v-if="overviewTotal > overviewLimit"
               :href="makeSearchURL([label, overviewSub])"
               class="btn btn-outline-primary btn-sm rounded-pill mt-3">
               一覧を見る
            </a>
          </div>

          <!-- 新着 -->
          <div class="mb-4">
            <div class="p-3 bg-light border-start border-success rounded">
              <h2>{{ label }} に関する新着投稿</h2>
            </div>

            <div class="label-row">
              <span class="post-labels-title">投稿ラベル</span>
              <label-box :labels="[label]"></label-box>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3">
              <button type="button"
                      class="btn btn-outline-success btn-sm rounded-pill"
                      @click="copyAndNavigate([label])">
                新しい投稿記事を書いてここに表示する
              </button>
              <button type="button"
                      class="btn btn-outline-secondary btn-sm rounded-pill"
                      @click="openHelpModal(label)">
                ヘルプ
              </button>
            </div>

            <div v-if="latestTotal > latestLimit">
              <a :href="makeSearchURL([label])"
                 class="btn btn-outline-primary btn-sm rounded-pill mb-3">
                 一覧を見る
              </a>
            </div>

            <div class="row row-cols-1 row-cols-md-2 g-3">
              <div v-for="item in latest" :key="item.id" class="col">
                <div class="card h-100">
                  <div class="card-body">
                    <h5 class="card-title">
                      <a :href="item.link" class="fw-bold text-primary text-decoration-none">
                        <span v-if="item.categories.includes('重要')" class="badge-pill-important">重要</span>
                        <span v-if="item.categories.includes('基本')" class="badge-pill-basic">基本</span>
                        {{ item.title }}
                      </a>
                    </h5>
                    <div class="text-muted small mb-2">
                      <i class="bi bi-calendar3 me-1"></i>{{ formatDate(item.published) }}
                    </div>
                    <p class="card-text small text-secondary">{{ item.content.slice(0, 80) }}...</p>
                  </div>
                </div>
              </div>
            </div>

            <a v-if="latestTotal > latestLimit"
               :href="makeSearchURL([label])"
               class="btn btn-outline-primary btn-sm rounded-pill mt-3">
               一覧を見る
            </a>
          </div>

          <!-- 重要 -->
          <div class="mb-4">
            <div class="p-3 bg-light border-start border-danger rounded">
              <h2>{{ label }} に関する最近の重要な投稿</h2>
            </div>

            <div class="label-row">
              <span class="post-labels-title">投稿ラベル</span>
              <label-box :labels="[label, importantSub]"></label-box>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3">
              <button type="button"
                      class="btn btn-outline-success btn-sm rounded-pill"
                      @click="copyAndNavigate([label, importantSub])">
                新しい投稿記事を書いてここに表示する
              </button>
              <button type="button"
                      class="btn btn-outline-secondary btn-sm rounded-pill"
                      @click="openHelpModal(label + ' と ' + importantSub)">
                ヘルプ
              </button>
            </div>

            <div v-if="importantTotal > importantLimit">
              <a :href="makeSearchURL([label, importantSub])"
                 class="btn btn-outline-primary btn-sm rounded-pill mb-3">
                 一覧を見る
              </a>
            </div>

            <div class="row row-cols-1 row-cols-md-2 g-3">
              <div v-for="item in important" :key="item.id" class="col">
                <div class="card h-100">
                  <div class="card-body">
                    <h5 class="card-title">
                      <a :href="item.link" class="fw-bold text-primary text-decoration-none">
                        <span v-if="item.categories.includes('重要')" class="badge-pill-important">重要</span>
                        <span v-if="item.categories.includes('基本')" class="badge-pill-basic">基本</span>
                        {{ item.title }}
                      </a>
                    </h5>
                    <div class="text-muted small mb-2">
                      <i class="bi bi-calendar3 me-1"></i>{{ formatDate(item.published) }}
                    </div>
                    <p class="card-text small text-secondary">{{ item.content.slice(0, 80) }}...</p>
                  </div>
                </div>
              </div>
            </div>

            <a v-if="importantTotal > importantLimit"
               :href="makeSearchURL([label, importantSub])"
               class="btn btn-outline-primary btn-sm rounded-pill mt-3">
               一覧を見る
            </a>
          </div>

          <!-- 基本 -->
          <div class="mb-4">
            <div class="p-3 bg-light border-start border-danger rounded">
              <h2>{{ label }} に関する最近の基本的な投稿</h2>
            </div>

            <div class="label-row">
              <span class="post-labels-title">投稿ラベル</span>
              <label-box :labels="[label, basicSub]"></label-box>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3">
              <button type="button"
                      class="btn btn-outline-success btn-sm rounded-pill"
                      @click="copyAndNavigate([label, basicSub])">
                新しい投稿記事を書いてここに表示する
              </button>
              <button type="button"
                      class="btn btn-outline-secondary btn-sm rounded-pill"
                      @click="openHelpModal(label + ' と ' + basicSub)">
                ヘルプ
              </button>
            </div>

            <div v-if="basicTotal > basicLimit">
              <a :href="makeSearchURL([label, basicSub])"
                 class="btn btn-outline-primary btn-sm rounded-pill mb-3">
                 一覧を見る
              </a>
            </div>

            <div class="row row-cols-1 row-cols-md-2 g-3">
              <div v-for="item in basic" :key="item.id" class="col">
                <div class="card h-100">
                  <div class="card-body">
                    <h5 class="card-title">
                      <a :href="item.link" class="fw-bold text-primary text-decoration-none">
                        <span v-if="item.categories.includes('重要')" class="badge-pill-important">重要</span>
                        <span v-if="item.categories.includes('基本')" class="badge-pill-basic">基本</span>
                        {{ item.title }}
                      </a>
                    </h5>
                    <div class="text-muted small mb-2">
                      <i class="bi bi-calendar3 me-1"></i>{{ formatDate(item.published) }}
                    </div>
                    <p class="card-text small text-secondary">{{ item.content.slice(0, 80) }}...</p>
                  </div>
                </div>
              </div>
            </div>

            <a v-if="basicTotal > basicLimit"
               :href="makeSearchURL([label, basicSub])"
               class="btn btn-outline-primary btn-sm rounded-pill mt-3">
               一覧を見る
            </a>
          </div>

          <!-- ヘルプモーダル -->
          <div v-if="isHelpModalOpen" class="custom-modal-overlay" @click.self="closeHelpModal">
            <div class="custom-modal-content">
              <h4 class="mb-3">投稿手順のヘルプ</h4>
              <p>新しい投稿記事を作成して、ここに表示するには、投稿エディタを開いて <b>{{ label }}</b> と <b>{{ basicSub }}</b> をラベル欄に入力して公開してください。</p>
              <ol>
                <li>この[ヘルプ]のボタンの左側にある[新しい投稿記事を作成してここに表示する]ボタンをクリックしてください。</li>
                <li>Google Blogger にサインイン(ログイン)していない場合には、サインイン(ログイン)してください。</li>
                <li>ダッシュボード(管理画面)を開いて[投稿]を選択します。</li>
                <li>[+新しい投稿]ボタンをクリックして投稿エディタを開きます。</li>
                <li><b>{{ helpModalText }}</b> をカンマ(,)で区切ってラベルの欄に書き込みます。[新しい投稿記事を作成してここに表示する]ボタンをクリックしてから投稿エディタを開いた場合には、<b>{{ helpModalText }}</b> の文字列が自動的にクリップボードにコピーされていますので、ラベルの欄に貼り付けて(ペーストして)ください。</li>
                <li>続いて、記事のタイトルと本文を書きます。</li>
                <li>書き終わったら、[公開]ボタンをクリックして公開してください。</li>
              </ol>
              <div class="text-end">
                <button type="button" class="btn btn-primary btn-sm px-4 rounded-pill" @click="closeHelpModal">
                  OK
                </button>
              </div>
            </div>
          </div>

        </div>
      `
    };

    /* ---------------------------------------------------------
     * ⑦ feed-section タグごとに Vue をマウント
     * --------------------------------------------------------- */
    targets.forEach((el) => {
      const props = {};
      for (const attr of el.attributes) {
        props[attr.name] = attr.value;
      }
      createApp(FeedSection, props).mount(el);
    });

  }, 50);

})();
